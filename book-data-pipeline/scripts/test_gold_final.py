#!/usr/bin/env python3
"""
Working test for Gold layer pipeline
"""
...
from pyspark.sql.functions import lit, col, current_timestamp, mean
...
def test_gold_working():
    """Test the Gold layer pipeline with working data"""
    ...
    try:
        ...
        if books_toscrape is None:
            print("   ❌ No Silver layer data found.")
            # Create sample data for testing
            
            sample_data = [
                ("book1", "Travel Guide", ["Author A"], 29.99, "GBP", "Books > Travel", "http://example.com/1"),
                ("book2", "Python Programming", ["Author B"], 39.99, "USD", "Books > Technology", "http://example.com/2"),
            ]
            schema = StructType([
        ...
        sample_data = [
            ("9781912047451", "Python Programming Guide", ["John Smith"], 39.99, "USD", "Books > Technology", "http://example.com/python"),
            ("9781234567890", "Data Engineering Fundamentals", ["Jane Doe"], 49.99, "USD", "Books > Technology", "http://example.com/dataeng"),
            ("9781111111111", "Travel Guide 2024", ["Author A"], 29.99, "USD", "Books > Travel", "http://example.com/travel"),
        ]
        ...
        canonical_path = gold_storage.save_clean_books(
            canonical_books, "canonical_books", "test"
        )
        
        # Save price history
        price_path = gold_storage.save_clean_books(
            price_history, "price_history", "test"
        )
        ...
        print("7. 🔍 Verifying saved data...")
        saved_canonical = spark.read.parquet(str(canonical_path))
        saved_prices = spark.read.parquet(str(price_path))
        
        print(f"   ✅ Saved canonical books: {saved_canonical.count()} records")
        print(f"   ✅ Saved price history: {saved_prices.count()} records")
